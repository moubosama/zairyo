...
const PARTITION_WALL_BANDS = {
  ...
  '3LDK': { min: 18, max: 30, fallback: 22 }, // アルファステイツ67戸平均≒22m。Gタイプ正解≒19.8mのためmin=18（20だと正読が誤警告になる）
...
export function validateAndNormalize(raw, options = {}) {
  ...
  // ---------- 2.5 未ラベル部屋の按分補正 ----------
  // 総面積が確定している場合、部屋合計は内法面積（≒専有×0.96）を超えないはず。
  // 超過分はラベルの無い部屋（廊下・キッチン等のAI目測）の読みすぎなので、
  // ラベル部屋は固定したまま目測部屋だけを按分縮小する
  {
    const total = data.total_floor_area_sqm;
    const trustedTotal = ['user_input', 'ai_label_roomsum_verified', 'ai_estimate_verified'].includes(data.total_area_source);
    if (total && trustedTotal && Array.isArray(data.rooms) && data.rooms.length > 0) {
      const roomSum = data.rooms.reduce((s, r) => s + (r.area_sqm || 0), 0);
      const target = total * 0.96; // 内法面積の近似
      if (roomSum > target) {
        const labeledSum = data.rooms
          .filter((r) => r.area_source === 'sqm_label' || r.area_source === 'jou_label')
          .reduce((s, r) => s + (r.area_sqm || 0), 0);
        const estimateSum = roomSum - labeledSum;
        if (estimateSum > 0) {
          // 縮小率は0.5を下限（それ以上縮めるならラベル側の異常を疑うべき）
          const scale = Math.max(0.5, (target - labeledSum) / estimateSum);
          if (scale < 1) {
            for (const room of data.rooms) {
              if (room.area_source !== 'sqm_label' && room.area_source !== 'jou_label' && room.area_sqm) {
                room.area_sqm = Math.round(room.area_sqm * scale * 100) / 100;
              }
            }
            warnings.push({
              field: 'rooms',
              message: `部屋面積合計(${Math.round(roomSum * 10) / 10}㎡)が総面積${total}㎡の内法相当(${Math.round(target * 10) / 10}㎡)を超過。ラベルの無い部屋の目測を${Math.round(scale * 100)}%に按分縮小`,
              before: Math.round(roomSum * 10) / 10,
              after: Math.round((labeledSum + estimateSum * scale) * 10) / 10,
            });
          }
        }
      }
