...
- wall = その記号が付いている壁が、その部屋の平面形状から見て「長辺」側か「短辺」側か。
  部屋がほぼ正方形・タイル切れで判断できない場合は null（推測禁止）
- 図面右欄の凡例表の中の記号は対象外（実際の壁面に付いたものだけ）
- WD-2TAのような建具符号、通り芯記号、寸法は対象外

以下のJSONのみを返す（説明不要）:
{"codes": [{"room": "洋室(1)", "code": "C04", "wall": "長辺"}, {"room": "洋室(1)", "code": "I14", "wall": "短辺"}]}
...
export async function analyzeWallCodesTiled(filePath, context = {}) {
  ...
  // 部屋ごとにユニーク化して集約（記号+長辺/短辺の割付情報も保持）
