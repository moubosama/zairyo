...
async function analyzeTiles(filePath, prompt, resultKey) {
  ...
  let failedTiles = 0;
  const results = await Promise.all(tiles.map((tile, i) =>
    analyzeSingle(filePath, tile.base64Data, tile.mimeType, prompt)
      ...
      .catch((e) => {
        failedTiles++;
        console.warn(`タイル${i + 1}/${tiles.length} 解析失敗:`, e?.message);
        return [];
      })
  ));
  return { results: results.flat(), failedTiles, totalTiles: tiles.length };
}
