...
import {
  collapseDoubledPlacements, normalizeRoomName, parseWallCode, canonicalizeWallCode,
...
■ 壁仕上記号の転記（平面詳細図の各壁面に付く記号）
  - 物件により2方式のどちらか。図面の凡例表を見て、使われている方を転記する:
    * 方式1: 楕円内の「英字1文字+数字2桁」（例: D14, C04, I14, L14, G24）
    * 方式2: 丸囲み/四角囲みの1文字を引き出し線で壁に向けたもの（例: Ａ, Ｂ, Ｃ, Ｇ, 遮）
  - JSONの wall_finish_codes に [{"room": "洋室(1)", "codes": ["I14", "C04"]}] の形式で入れる
  - 方式2でＧが四角囲みの場合は "G枠" と書く（丸囲みのＧ="G"とは別の仕上げのため区別が必要）
  - **天井伏図（別枠の平面図）の中の丸囲み記号は天井の仕上げ記号なので転記しない**
    （壁の記号は引き出し線で壁を指す。天井の記号は部屋の中央に置かれる）
...
【展開図の読み方】
- 1部屋につきA〜D面（4方向の壁）の立面が並んでいる。面ラベルはＡ・Ｂ・Ｃ・Ｄ（作図者により時計回り等・向きの規則は物件依存なので気にしない）
...
- **1ページに全室の立面が詰め込まれ、上段の大きな居室（LDK・洋室）の下段や右端に、
  水回り（浴室・UB・洗面・トイレ）・玄関・収納の小さな立面が配置されることが多い。
  ページ全体（下段・右端まで）を走査し、段や列をまたいで全ての室の立面を漏れなく拾うこと。
  上段の数室だけで打ち切らない**
...
ルール:
- roomsにはページ内の全部屋を列挙（玄関・廊下、トイレ、キッチン、パウダールーム、浴室・UB・ユニットバス、洗面、収納等も省略しない）。
  立面が描かれている室を1つも飛ばさないこと（下段・右端の小さい室を落としやすいので特に注意）
...
- wall_code: 面に壁仕上記号が読める場合のみ転記。無ければnull。物件により2方式のどちらか:
  （方式1）D14・C04・I14のような「英字1+数字2」の楕円記号
  （方式2）Ａ・Ｂ・Ｃ・Ｄ・Ｅ・Ｆ・Ｇ・遮のような丸囲み1文字（1文字だけの記号）。
  方式2では戸境壁（隣戸との界壁）が重要で、その面に「遮」（丸囲み・戸境二重遮音壁）や
  四角囲みのＧ（＝ "G枠" と書く。戸境二重壁。丸囲みのＧとは別の仕上げ）が付く。
  戸境壁はLDK・洋室の外周側の1面に付くことが多いので、その面の記号を落とさず転記すること。
  どちらの方式かは図面に描かれている記号をそのまま読めばよい（記号が読めた面だけ転記・読めなければnull・捏造禁止）
...
壁仕上記号の書き方は物件によって次の2方式のどちらかです。**この図面で使われている方だけ**を転記してください
（両方式が混在することはありません。図面の凡例表を見て判断する）。

■方式1: 3桁記号（例: D14, C04, I14, L14, G24, D64）
  - 壁面の近くにある楕円の中の「英字1文字+数字2桁」
  - 業務知識（読み落とし防止）:
...
■方式2: 1文字記号（例: Ａ, Ｂ, Ｃ, Ｄ, Ｅ, Ｆ, Ｇ, 遮）
  - 丸囲みまたは四角囲みの1文字を、**引き出し線でその壁を指して**書く
  - 凡例の例: Ａ=ビニルクロス貼+PBt9.5 / Ｂ=同（GL工法） / Ｃ〜Ｅ,Ｇ=コンクリート打放し補修 /
    Ｆ=LGS下地+ケイカル板 / 遮=戸境二重遮音壁 / Ｇ(四角囲み)=戸境二重壁
  - **囲みの形（丸/四角）を shape に必ず入れる**（"circle" または "box"）。
    Ｇは丸囲みと四角囲みで意味が全く違う（丸=打放しのまま / 四角=戸境二重壁）ため、
    形が判別できないときは shape: null（推測禁止）
  - 遮は戸境壁（クロスハッチングで塗られた厚い壁）を指していることが多い

【共通ルール】
- 記号がどの部屋のどの壁に付いているか、周囲の室名表記と壁の位置から判定する
- 同じ記号が同じ部屋の複数の壁に付くことも多い。全部列挙する
...
- **壁そのものの長さ寸法だけ**を転記すること。次のものは壁長ではないので使わない:
  * 器具寸法（洗面台のW・UBサイズ1416等）・開口（ドア・窓）の寸法・通り芯間の細かい寸法
  * **壁の厚み寸法**（記号の引き出し線の先の壁に「150」「100+50」「200」のように
    書かれる小さい数値。これは壁の厚さであって長さではない）

【対象外（転記してはいけないもの）】
- 図面右欄・下部の凡例表の中の記号（実際の壁面に付いたものだけが対象）
- **天井伏図の中の記号**: 天井伏図（「天井伏図」「S=1:75」等の見出しが付いた別の平面図）にも
  同じＡ〜Ｇの丸囲み記号が出るが、それは**天井の仕上げ記号**であって壁の記号ではない。
  見分け方: 天井の記号は室名や天井高（2,500等）の**すぐ下の部屋の中央**に置かれ、
  引き出し線で壁を指していない。壁の記号は引き出し線で壁を指す
- WD-2TAのような建具符号、通り芯記号

以下のJSONのみを返す（説明不要）:
{"codes": [
  {"room": "洋室(1)", "code": "C04", "wall_length_mm": 5190, "shape": null},
  {"room": "洋室(1)", "code": "I14", "wall_length_mm": 2575, "shape": null},
  {"room": "洋室3", "code": "遮", "wall_length_mm": 3250, "shape": "circle"},
  {"room": "洋室3", "code": "G", "wall_length_mm": 2250, "shape": "box"}
]}
...
export function aggregateWallCodeItems(raw, opts = {}) {
  ...
  for (const item of raw) {
    ...
    // 囲み形状（別府方式のＧの二義性の解決に使う）を記号文字列に畳み込んでから検証する。
    // 保存されるのは code 文字列だけなので、四角囲みのＧは正規表記 'G枠' へ確定させて
    // 「戸境二重壁」の意味を持たせる（丸囲み・形状不明のＧは打放し=既定のまま）。
    // こうすることで shape を保存データのスキーマに増やさずに済み、
    // 再計算（リプレイ・/calculate）でも同じ意味に解決される
    const code = canonicalizeWallCode(item.code, item.shape);
    // 有効な壁仕上記号のみ通す。アルファ方式の3桁（G14/C04…）に加え、
    // 別府方式の1文字（Ａ〜Ｇ・遮・G枠）も受ける（parseWallCodeが両方を判定する単一の門番）。
    // 旧実装は /^[A-Z][0-9][0-9]$/ のハードコードで、別府記号が集約段で全て捨てられていた
    // （＝記号ゼロ→全面デフォルトPB扱い→壁PB暴発の経路）
    //
    // 【挙動変化の記録（should-fix②・2026-07-24レビュー指摘）】この差し替えは別府記号を
    // 通すだけでなく**門番を厳しくもしている**: 旧正規表現は下地英字を検証していないため
    // 凡例に存在しない 'T14' / 'X99' 等の3桁も集約を通していた。新門番のparseWallCodeは
    // VALID_BASE_CODES='ZCDGHILSOW' で弾くため、これらはここで落ちる。
    // 下流影響はゼロ（旧経路でも面割付時のparseWallCodeがnullを返し無視されていた）だが、
    // 「集約結果のplacements件数」は変わりうる＝アルファ完全不変とは言い切れない差分のため明記する。
    // 実データのアルファ記録3件には該当記号が無く、eval/replayの数値は不変（検証済み）。
    // 固定テスト: scripts/test-beppu-wallcode.mjs C12/C13
    if (!code || !parseWallCode(code)) continue;
...
function roomContextNote(roomNames) {
  ...
  return `\n\n【この住戸の部屋一覧（平面詳細図の読み取り結果・確定情報）】\n${roomNames.join('、')}\n` +
    ...
    '「洋室1」と「洋室(1)」のような全角括弧・半角の表記ゆれも一覧の表記に統一する）\n' +
    '- 一覧にある部屋がページ内に描かれていれば読み落とさないこと（一覧に無い室があれば図面の表記のまま追加してよい）\n' +
    '- **一覧の各室について、展開図に立面（A〜D面）が描かれていないか必ず探すこと。' +
    '展開図は複数段・複数列に分かれて配置され、居室→水回り→収納の順で下段や右側に小さく描かれることが多い。' +
    '上段の大きな居室だけを拾って下段・右端の室（浴室・UB・ユニットバス・洗面・トイレ・収納・玄関）を読み落とさないこと**\n' +
    '- 実際に立面が描かれている室のみroomsに入れる（一覧にあっても展開図に立面が無い室＝棚スペース・冷蔵庫置場・パントリー等は無理に作らない。捏造禁止）';
}
