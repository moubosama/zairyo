...
// このファイルを直接実行したときだけ eval 本体を走らせる（他スクリプトが
// フィクスチャ G_ELEVATIONS 等を import しても副作用（実行・process.exit）が起きないように）。
...
// ============ 材積換算（m³・従来パス=平面図のみ） ============
// 【2026-07-25 期待値是正】材積の期待値を「見積明細÷67戸（A〜H全タイプ平均）」から
// XLS集計表(②G.XLS)のG単体正解（Y列/戸=拾い量×材積係数X列）へ是正。②G.XLSは C列=Gタイプ戸当
// （現場名等!A4=G・B4=9戸）で、各行の Y=W×X が材積（W=戸当×9戸）＝ G戸当は C×X。
// 67戸平均はGより+18〜20%上振れ（Gは小さめのタイプ）で誤基準だった（KP+47%と同型の期待値監査漏れ）。
//   間仕切木軸 旧1.1537(=77.3/67) → 0.975351（集計表 C52=84.082㎡ × X52=0.0116m³/㎡）
//   天井下地   旧0.5746 (=38.5/67) → 0.478608（集計表 C77=59.0874㎡ × X78=0.0081m³/㎡）
//   木胴縁     旧0.0537 (=3.6/67)  → 0.049461（集計表 C85=界壁面5.047㎡ × X86=0.0098m³/㎡）
//   際根太     0.0246（=C9=18.2m × X9=0.00135m³/m）は元々G単体で正しく是正不要
// 逆算でない証拠: 是正後もエンジン出力は 際根太+4.6% の残差が残る
//   （エンジンに合わせるなら際根太0.0257を採るところ、XLS数式Y=W×Xから導いた値に是正）。
// tolは緩めず、67戸平均で覆い隠していた35/45を材積係数がXLS一致した今 10 へ厳格化する。
//
// 【2026-07-25 案B: 材積係数をXLS X列へ整合（材長×断面→拾い面積×材積係数の直接方式）】
//   間仕切木軸: 実効係数を 0.0120(+3.4%)→ MAJIKIRI_M3_PER_SQM=0.0116（XLS X52・原典一致）へ。
//     材積は推定パスで 拾い84.082×0.0116=0.975（±0%）/ 適用後は展開図実測77.5×0.0116=0.899（-7.8%）。
//     ※係数を直したことで相殺（旧+3.4%×拾い-7.8%）が外れ、適用後材積は拾い面積残差-7.8%が素で出る。
//       拾い面積-7.8%は展開図読み取り層の構造差でスコープ外＝残す（±10%内・test-timber-volume-coeff で固定）。
//   天井下地: 実効係数を 0.008457(+4.4%)→ CEILING_M3_PER_SQM=0.0081（XLS X78・原典一致）へ。
//     材積は 天井57.72×0.0081=0.4675（拾い-2.3%が素で出る・±10%内）。
const EXPECTED_VOLUME = [
  ['際根太 材積',       0.024570, 'm³', (mats) => qtyM3(mats, '際根太'), 12], // 集計表 C9=18.2×X9=0.00135
  ['間仕切木軸 材積',   0.975351, 'm³', (mats) => qtyM3(mats, '間仕切木軸'), 10], // C52=84.082×X52=0.0116
  ['木胴縁 材積',       0.049461, 'm³', (mats) => qtyM3(mats, '木胴縁'), 10], // C85=5.047×X86=0.0098
  ['天井下地 材積',     0.478608, 'm³', (mats) => qtyM3(mats, '天井下地'), 10], // C77=59.0874×X78=0.0081
];

// ============ 展開図実測適用後（applyElevationTakeoff） ============
const EXPECTED_APPLIED = [
  ['間仕切下地(木)適用後', 84.082, 'm',  (mats) => qty(mats, '間仕切下地(木)'), 10],
  // 【2026-07-25 期待値是正】適用後も67戸平均1.1537→G単体0.975351（集計表 C52=84.082×X52=0.0116）。
  // 適用後は展開図実測（間仕切下地-7.8%）に材積係数を掛けるため推定パス(+3.4%)より小さく-4.7%になる。
  // これはXLS原典値との「残差」＝逆算でない証拠（完全一致させるなら0.9299を採る）。tol 35→10へ厳格化。
  ['間仕切木軸 適用後',   0.975351, 'm³', (mats) => qtyM3(mats, '間仕切木軸'), 10], // C52×X52
  // 木胴縁（一部界壁面）: XLS集計表r85 = 界壁面5.047㎡/戸 × X86=0.0098 = 0.049461m³/戸。
  // 見積明細の3.6m³/67戸=0.0537（A〜H全タイプ平均）とは-7.9%差で、G単体の原典値の方が小さいのが正。
  // 旧版は tol=999（=判定放棄）で「界壁・EV面が展開図外・Gは界壁C04で胴縁なし」と注記していたが、
  // XLS原典にGの界壁面5.047㎡が実在し**胴縁ありが正**（2026-07-24是正）。実測なし時は推定を維持する
  // 設計のため適用後も推定値と一致する。他部位と同じ±10%で判定する
  ['木胴縁 適用後',       0.049461, 'm³', (mats) => qtyM3(mats, '木胴縁'), 10], // C85×X86
];
