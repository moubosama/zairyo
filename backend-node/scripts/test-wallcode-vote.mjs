...
const {
  ...
  buildWallCodeReadPlan, runWallCodePlan,
...
// 非同期テスト（runWallCodePlan等）。top-level await可能なモジュール内で await して使う
...
test('有効2runの1/2は寸法付きでは不採用・codesのみへ降格（Fix3: 面割付は厳格・記号存在は緩い）', () => {
  // 2026-07-21 Fix3で挙動変更: 寸法付き（=面割付に使う・壁PB過大に直結）は厳格な過半数
  // floor(有効/2)+1 を要求 → 有効2runの1/2（votes=1）は不採用。片run幻覚の寸法が面割付されて
  // 壁PBを膨らませる事故を防ぐ。ただし「読めた事実の尊重」は codesのみ（寸法null・面割付しない）
  // には残す（loose=ceil(有効/2)=1 で存在記録は残る）。
  const res = voteWallCodeRuns([
    { items: [it('トイレ', 'G24', 950, 0)], failedTiles: [] },
    { items: [], failedTiles: [] },      // タイル0は読めたが記号なし → 反対票
    { items: [], failedTiles: [0] },     // タイル0全滅 → 分母から除外
  ]);
  const e = roomOf(res, 'トイレ');
  // 有効2run・出現1: 寸法付き厳格しきい値2に届かず面割付されない。だが codesのみ loose=1 で存在は残る
  assert.deepEqual(e.codes, ['G24']);
  assert.deepEqual(e.placements, [{ code: 'G24', wall_length_mm: null }],
    '寸法付きは落選・記号の存在はcodesのみで残す');
});
...
test('片AI3/6は寸法付きでは不採用（Fix3: 1/2ちょうどのタイは面割付しない）・codesのみへ降格', () => {
  // 2026-07-21 Fix3で挙動変更: 3/6は「厳密な過半数」ではなく1/2ちょうどのタイ。
  // 面割付する寸法付きは厳格しきい値 floor(6/2)+1=4 を要求 → votes=3<4 で不採用。
  // 片AIだけが一致し他AIが読み漏れた壁を、寸法付き（過大リスク）で確定させない。
  // 記号の存在自体は codesのみ loose=ceil(6/2)=3 で残る（存在の合図は保持）。
  const res = voteWallCodeRuns([
    gem(it('洋室(3)', 'C04', 2360)), gem(it('洋室(3)', 'C04', 2360)), gem(it('洋室(3)', 'C04', 2360)),
    cla(), cla(), cla(), // Claudeはこの壁を読み漏れ（記号なし=反対票）
  ]);
  const e = roomOf(res, '洋室(3)');
  assert.deepEqual(e.codes, ['C04']);
  assert.deepEqual(e.placements, [{ code: 'C04', wall_length_mm: null }],
    '3/6は寸法付き落選・codesのみで存在は残す');
});
...
// ---------------------------------------------------------------------------
console.log('■ runWallCodePlan: 全滅プロバイダのスキップ・null時break（Fix4/Fix7）');

// タイル読取結果のスタブ（analyzeTilesの返り値の要点だけ）
...
await asyncTest('Fix4: 全滅でない部分失敗（1タイルだけ失敗）は継続する', async () => {
  const calls = [];
  const partial = (label) => ({ results: [], failedTiles: 1, totalTiles: 6, _label: label });
  const plan = [{ provider: 'gemini', count: 3 }];
  const { runs } = await runWallCodePlan(plan, ({ provider, k }) => {
    calls.push(`${provider}${k}`);
    return Promise.resolve(partial(`${provider}${k}`));
  });
  assert.deepEqual(calls, ['gemini0', 'gemini1', 'gemini2'], '部分失敗は全滅ではないので継続');
  assert.equal(runs.length, 3);
});

await asyncTest('Fix7: readOneRunがnull（タイル分割不可）を返したら外側プロバイダループも抜ける', async () => {
  const calls = [];
  const plan = [{ provider: 'gemini', count: 3 }, { provider: 'claude', count: 3 }];
  const { runs, tilesUnavailable } = await runWallCodePlan(plan, ({ provider, k }) => {
    calls.push(`${provider}${k}`);
    // 1runだけ読めた後にnull（makeTiles失敗）→ 以降はGeminiもClaudeも呼ばれない
    if (calls.length === 1) return Promise.resolve(okRun(`${provider}${k}`));
    return Promise.resolve(null);
  });
  assert.deepEqual(calls, ['gemini0', 'gemini1'], 'null検知で外側ループも打ち切り（Claudeは呼ばれない）');
  assert.equal(runs.length, 1, '読めた1runは保持');
  assert.equal(tilesUnavailable, true);
});

await asyncTest('Fix7: 最初のrunがnull → runs空・tilesUnavailable（呼び出し元でnull返しに使う）', async () => {
  const { runs, tilesUnavailable } = await runWallCodePlan(
    [{ provider: 'gemini', count: 3 }], () => Promise.resolve(null));
