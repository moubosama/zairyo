...
def extract_page(page):
    ...
    dims, rooms, wall_codes, door_words, others = [], [], [], [], []
    for w in words:
    ...
    return dims, rooms, wall_codes, door_symbols
        ...
        for band in bands:
            ...
            for d in band[1:]:
                if run(d) - run_end(cur[-1]) <= CHAIN_GAP:
                    ...
                    if len(cur) >= 2:
                        chains.append({'axis': 'horiz' if axis == 1 else 'vert',
                                       'values': [x['value_mm'] for x in cur]})
                    cur = [d]
            if len(cur) >= 2:
                chains.append({'axis': 'horiz' if axis == 1 else 'vert',
                               'values': [x['value_mm'] for x in cur]})
...
def main():
    ...
    for pn in pages:
        page = doc[pn - 1]
        dims, rooms, wall_codes, door_symbols = extract_page(page)
